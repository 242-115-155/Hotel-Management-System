...
<head>
    ...
    <style>
        * {
        ...
        /* Style fallback based on status texts */
        .status-badge {
        ...
        .cancel-form-fields input[type="number"] {
            padding: 12px 16px;
            font-size: 14px;
            color: #231F1B;
        ...
        .cancel-form-fields input[type="number"]:focus {
            border-color: #c94b4b;
        ...
        .btn-cancel:hover {
            background-color: #c94b4b; /* Changes to crimson warning on hover */
            color: #FFFFFF;
            box-shadow: 0 5px 15px rgba(201, 75, 75, 0.15);
        }
    </style>
</head>
<body>
    ...
    <div class="main-content">
        ...
        <div class="page-header">
            <div>
                <h1>Booking Logs</h1>
                <p>Monitor your active reservation approvals and processing status live.</p>
            </div>
        ...
        <!-- Live Status Table Card -->
        <div class="card">
            <h2 class="card-title">Active Room Booking Status</h2>
                        ...
                        if ($result1 = mysqli_query($conn, $sql1)) {
                            while ($row = mysqli_fetch_row($result1)) {
                                if($phone == $row[0] && $idproof == $row[2]) {
                                    ?>
                                    <tr>
                                        <td class="highlight-id"><?php echo htmlspecialchars($row[15]); ?></td>
                                        <td><?php echo htmlspecialchars($row[1]); ?></td>
                                        <td><?php echo htmlspecialchars($row[3]); ?></td>
                                        <td><?php echo htmlspecialchars($row[4]); ?></td>
                                        <td><?php echo htmlspecialchars($row[5]); ?></td>
                                        <td>
                                            <span class="status-badge"><?php echo htmlspecialchars($row[13]); ?></span>
                                        </td>
                                        <td class="highlight-price">BDT <?php echo htmlspecialchars($row[14]); ?></td>
                                    </tr>
                                    <?php
                                }
                            }
        ...
        <!-- Cancellation Form Card Section -->
        <div class="card" style="padding: 30px;">
            <div class="cancel-box">
                <div class="cancel-text">
                    <h3>Need to Cancel a Booking?</h3>
                    <p>Enter your unique Booking ID below to terminate a pending request.</p>
                </div>
                <form action="user_cancel_room.php" method="post" class="cancel-form-fields">
                    <input type="number" name="book_id" placeholder="e.g. 1024" required>
